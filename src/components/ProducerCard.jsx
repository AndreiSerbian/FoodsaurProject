        ...
        <div className="space-y-4">
          <div className="h-48 rounded-lg bg-gray-100 flex items-center justify-center relative overflow-hidden">
            {producer.producerImage ? (
              <>
                <img 
                  src={showExterior ? producer.producerImage.exterior : producer.producerImage.interior} 
                  alt={`${producer.producerName} ${showExterior ? 'exterior' : 'interior'}`} 
                  className="h-full w-full object-cover rounded-lg"
                  onError={handleImageError}
                />
                <button 
                  onClick={toggleProducerImage} 
                  className="absolute bottom-2 right-2 bg-white/80 text-xs px-2 py-1 rounded-full text-gray-800 hover:bg-white"
                >
                  {showExterior ? 'Интерьер' : 'Экстерьер'}
                </button>
              </>
            ) : images.length > 0 ? (
              <img 
              ...
              <img
                src="/placeholder.svg"
                alt={`${producer.producerName} placeholder`}
                className="h-full w-full object-cover rounded-lg"
              />
            )}
            
            {images.length > 1 && !producer.producerImage && (
              <>
